...
from varshil.GAIL import run_mujoco
...
def argsparser():
    ...
    parser.add_argument('--env', type=str, default='FetchPickAndPlace')    ######Change for fetch env## 
...
def evaluate_env(env_name, seed, policy_hidden_size, stochastic, reuse, prefix):
    ...
    def get_checkpoint_dir(checkpoint_list, limit, prefix):
        for checkpoint in checkpoint_list:
            if ('limitation_' in checkpoint) and (prefix in checkpoint): #+str(limit)   
                return checkpoint
        return None
    ...
    data_path = os.path.join('data', 'deterministic.trpo.' + env_name + '.0.01.npz')		##dataset loading # make sure it
    ...
    for i, limit in enumerate(CONFIG['traj_limitation']):
        ...
        env = gym.make(env_name + '-v1')						## Need to change
        env.seed(seed)
        ...
        avg_len, avg_ret = run_mujoco.runner(env,
                                             ...
                                             reuse=((i != 0) or reuse))
        normalized_ret = 50+avg_ret/50+upper_bound			##My**
        #normalized_ret = upper_bound/avg_ret
